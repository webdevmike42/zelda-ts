...
export function loadHud(ctx: CanvasRenderingContext2D, srcPath: string): void {
    const hud = new Image();
    hud.src = srcPath;
...
function drawHudImage(srcX: number, srcY: number, width: number, height: number, hudX: number, hudY: number) {
...
export function createHudDtoFromGameObject(player: Player): HudDto {
...
export function drawHUD(dto:HudDto):void {
...
function drawTriforce(collectedTriForcePiecesArray:any[]) {
...
function drawRupees(rupees:number):void {
    drawCounterToHud(rupees, 96, 193, 8);
}

function drawCounterToHud(num:number, x:number, y:number, xOffset:number) {
    if (num < 100)
...
function drawHudDigit(digit:number, x:number, y:number) {
    drawHudImage(528 + (digit * 9), 117, 8, 8, x, y);
}

function drawKeys(keys:number) {
...
*/
function drawHealthBar(health:number, maxHealth:number):void {
    let xOffset = 8, rowOffset = -8, heartsPerRow = 8, heartsDrawn = 0,
        rowCount = Math.ceil(maxHealth / heartsPerRow),
        fullHearts = Math.floor(health),
        hasHalfHeart = fullHearts !== health;
...
function drawFullHeart(x:number, y:number) {
    drawHudImage(645, 117, 8, 8, x, y);
}

function drawEmptyHeart(x:number, y:number) {
    drawHudImage(627, 117, 8, 8, x, y);
}

function drawHalfHeart(x:number, y:number) {
...
function setHudOffset(offset:number) {
    _offset = offset;
}

export function moveHudUpBy(pixels:number) {
    setHudOffset(Math.max(getHudOffset() - pixels, MAX_HUD_OFFSET));
}

export function moveHudDownBy(pixels:number) {
